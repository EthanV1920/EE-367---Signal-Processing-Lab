...
def wavgen(dur, freq, rate, channels, amp, phase, delay, fpath_wave_out):
    ...
    for i in range(sample_count):
        working_sample = abs(amp * math.cos(w1 * (i - delay * rate) + (phase/360 * 2 * math.pi)))
...
def main():
    try:
        fpath_wav_out = 'Lab1-4.wav'
        wavgen(0.0625, 160, 16000, 1, 24576, 0, 0, fpath_wav_out)
    except:
